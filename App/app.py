...
try:
    ...
    prolog_file_path = os.path.join(base_dir, 'prolog_advice.pl')
    json_file_path = os.path.join(base_dir, 'results_dictionary.json')

    log_message(f"Prolog file path: {prolog_file_path}")
    log_message(f"JSON file path: {json_file_path}")

    # Check if files exist
    if not os.path.exists(prolog_file_path):
        raise FileNotFoundError(f"Prolog file not found: {prolog_file_path}")
    ...
    log_message("JSON file read successfully")

    # Use the Prolog file as needed
    # For example, if you are using a Prolog interpreter library,
    # you can load the Prolog file like this:
    # prolog_interpreter.load_file(prolog_file_path)
    janus.consult(prolog_file_path)
    log_message("Prolog file processed successfully")
...
def result():
    ...
    print("MBTI desc: ", config['mbti_types'][partner_mbti])
    ...
    mbti_body2 = tk.Label(root, text=f"{config['mbti_types'][partner_mbti]}", bg=bg_color, font=body_font)
    ...
    ll_body = tk.Label(root, text=f"{config['love_languages'][partner_love_language]}", bg=bg_color, font=body_font)
    ...
    interests_string = interests_string.replace('_', " ")
    body1 = tk.Label(root, text=f"{interests_string}", bg=bg_color, font=body_font)
    body1.place(relx=0.5, y=560, anchor=tk.CENTER)  
    body2 = tk.Label(root, text=f"{config['interests'][partner_interests[0]]}", bg=bg_color, font=body_font)
    ...
    values_string = values_string.replace('_', " ")
    body1 = tk.Label(root, text=f"{values_string}", bg=bg_color, font=body_font)
    body1.place(relx=0.5, y=720, anchor=tk.CENTER)  
    body2 = tk.Label(root, text=f"{config['values'][partner_values[0]]}", bg=bg_color, font=body_font)
